...
| BUG-011 | High | Mood classification inaccurate â kids/upbeat songs (e.g. ×××× ×××××××) sorted into Melancholy. Genre-to-mood mapping needs validation and improvement | Open | Apr 14 | Sprint 001 |
...
| BUG-001 | Token expiry mid-session â API calls silently fail after ~1hr | Added `spotifyFetch` wrapper with 401 handling + token expiry tracking + reconnect toast | Apr 14 | Sprint 001 |
...
| BUG-006 | Shared moodboard items have no Spotify URLs â clicking reloads page | Share view now uses `<div>` instead of `<a>` when URL is empty | Apr 14 | Sprint 001 |
| BUG-008 | Clipboard fallback broken â catch handler retries same failing API | Replaced with hidden textarea + `execCommand('copy')` fallback | Apr 14 | Sprint 001 |
